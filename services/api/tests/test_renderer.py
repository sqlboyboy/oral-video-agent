...
def test_build_postprocess_command_preserves_source_audio_and_fullscreen_pip(tmp_path):
    renderer = Renderer()
    ...
    filter_complex = command[command.index("-filter_complex") + 1]
    assert command[1:4] == ["-y", "-i", str(source)]
    assert "[0:a]anull[aout]" in filter_complex
    assert "[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1,setpts=PTS-STARTPTS[mainv]" in filter_complex
    assert "[1:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1,setpts=PTS-STARTPTS[pip]" in filter_complex
    assert "[mainv][pip]overlay=0:0:eof_action=pass" in filter_complex
    assert "subtitles=" in filter_complex
