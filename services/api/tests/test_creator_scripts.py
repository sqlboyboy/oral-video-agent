...
def test_bootstrap_advertises_creator_style_script_capability():
    response = client.get("/api/bootstrap")

    assert response.status_code == 200
    assert response.json()["features"]["creator_style_scripts"] is True
...
def test_placeholder_provider_analyzes_snapshot_and_generates_eight_unique_scripts():
    ...
    assert sum(item.title.startswith("装修") for item in items) <= 2
    assert all("装修" in item.script for item in items)
    assert all("\n" in item.script for item in items)
    assert all(
        not any(mark in item.script for mark in "，。！？：；")
        for item in items
    )


def test_placeholder_regeneration_returns_a_genuinely_new_batch():
    provider = PlaceholderCreatorScriptProvider()
    profile = provider.analyze_style(_snapshot())
    first_batch = provider.generate_scripts(
        profile,
        keyword="装修",
        count=8,
        duration_seconds=60,
    ...
    second_batch = provider.generate_scripts(
        profile,
        keyword="装修",
        count=8,
        duration_seconds=60,
        generation_round=2,
        exclude_titles=[item.title for item in first_batch],
        exclude_scripts=[item.script for item in first_batch],
    )

    assert {item.title for item in first_batch}.isdisjoint(
        {item.title for item in second_batch}
    )
    assert {item.script for item in first_batch}.isdisjoint(
        {item.script for item in second_batch}
    )
    assert not any("新解" in item.title for item in second_batch)
...
def test_regenerate_endpoint_reuses_style_profile_without_collecting_or_analyzing(monkeypatch):
    ...
    class RegenerateProvider:
        ...
        def generate_scripts(self, style_profile, **kwargs):
            ...
            assert kwargs["exclude_scripts"] == ["第一批旧文案"]
