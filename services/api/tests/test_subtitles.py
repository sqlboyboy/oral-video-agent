...
def test_generate_ass_adaptive_font_never_enlarges_small_base_font(tmp_path):
    output = tmp_path / "subtitle-small-font.ass"

    generate_ass(
        "abcdefghijklmnopqr",
        SubtitleStyle(font_size=12, max_chars_per_line=8),
        output,
    )

    text = output.read_text(encoding="utf-8")
    assert r"{\fs11}" in text
    assert r"\fs28" not in text
