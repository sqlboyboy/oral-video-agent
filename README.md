...
| `codex/android-apk-no-activation` | 当前安卓产品分支；Android 客户端、签名 APK 构建和配套云端服务 |

安卓构建入口为 `scripts/build_android_release.ps1`。维护与验证方式见 [分支管理说明](docs/branch-management.md)。下方早期 Windows 部署记录保留供参考，安卓默认直接连接云端服务。
