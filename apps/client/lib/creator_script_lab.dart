...
class CreatorScriptLabDialog extends StatefulWidget {
  ...
  final Future<void> Function(bool activationExpired)? onUnauthorized;
...
class _CreatorScriptLabDialogState extends State<CreatorScriptLabDialog> {
  ...
  final Set<String> _excludedTitles = <String>{};
  final Set<String> _excludedScripts = <String>{};
  bool _loading = false;
  String _error = '';
  ...
  Future<void> _generate({bool regenerate = false}) async {
    ...
    try {
      ...
      final excludedTitles = <String>{
        ..._excludedTitles,
        if (regenerate) ..._items.map((item) => item.title),
      }.where((item) => item.trim().isNotEmpty).take(100).toList();
      final excludedScripts = <String>{
        ..._excludedScripts,
        if (regenerate) ..._items.map((item) => item.script),
      }.where((item) => item.trim().isNotEmpty).take(100).toList();
      final requestBody = <String, dynamic>{
        ...
        if (regenerate) 'exclude_titles': excludedTitles,
        if (regenerate) 'exclude_scripts': excludedScripts,
      };
      final requestClient = widget.client ?? http.Client();
      ...
      try {
        ...
        if (widget.client == null) requestClient.close();
      }
      final decodedText = utf8.decode(response.bodyBytes);
      if (response.statusCode < 200 || response.statusCode >= 300) {
        final responseError = _responseError(decodedText, response.statusCode);
        final normalizedError = responseError.toLowerCase();
        final activationExpired = normalizedError.contains('activation') ||
            normalizedError.contains('软件激活');
        if (response.statusCode == 401 ||
            (response.statusCode == 403 && activationExpired)) {
          await widget.onUnauthorized?.call(activationExpired);
          throw Exception(
            activationExpired ? '软件激活状态已失效，请关闭弹窗后重新激活' : '云端登录已过期，请关闭弹窗后重新登录',
          );
        }
      ...
      setState(() {
        if (regenerate) {
          _excludedTitles
            ..clear()
            ..addAll(excludedTitles);
          _excludedScripts
            ..clear()
            ..addAll(excludedScripts);
        }
        _items = items;
  ...
  String _responseError(String responseText, int statusCode) {
    if (statusCode == 404) {
      return '当前创作服务版本过旧或接口未启动，请关闭弹窗并点击顶部刷新后重试';
    }
    try {
  ...
  Widget _detailAngle(String text) {
    return Container(
      width: double.infinity,
